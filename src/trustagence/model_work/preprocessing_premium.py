...
def preprocess_data_for_modeling(
    ...
    try:
        ...
        if "date_birth" in df_processed.columns:
            df_processed["driver_age"] = (
                datetime.now()
                - pd.to_datetime(df_processed["date_birth"], errors="coerce")
            ).dt.days / 365.25
            df_processed = df_processed[
                (df_processed["driver_age"] >= 18) & (df_processed["driver_age"] <= 90)
            ]

        # Creating Vehicle_Age
        if "year_matriculation" in df_processed.columns:
            current_year = datetime.now().year
            df_processed["vehicle_age"] = current_year - pd.to_numeric(
                df_processed["year_matriculation"], errors="coerce"
            )
        ...
        if "date_driving_licence" in df_processed.columns:
            df_processed["driving_experience"] = (
                datetime.now()
                - pd.to_datetime(df_processed["date_driving_licence"], errors="coerce")
            ).dt.days / 365.25
        ...
        # S'assurer que la cible est numérique (pour 'had_claim' ou 'premium')
        df_processed[target_column] = pd.to_numeric(
            df_processed[target_column], errors="coerce"
        )
        ...
        # S'assurer que TOUTES les colonnes non-feature sont supprimées
        cols_to_drop = [
            "id",
            "date_start_contract",
            "date_last_renewal",
            ...
            "n_claims_year",
            "cost_claims_year",
            "n_claims_history",  # On les supprime, ils ont pollué le préprocesseur
            "r_claims_history",  # On les supprime, ils ont pollué le préprocesseur
            "lapse",
        ]
        ...
        if X_train_processed.shape[1] != len(all_feature_names):
            print("Warning: Shape mismatch. Adjusting feature names.")
            all_feature_names = [
                col for col in all_feature_names if col in X.columns or "_" in col
            ]
            if X_train_processed.shape[1] != len(all_feature_names):
                raise ValueError(
                    f"Shape mismatch: {X_train_processed.shape[1]} != {len(all_feature_names)}"
                )
