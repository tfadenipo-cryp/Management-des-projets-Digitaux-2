...
-------------------------------------------------
This script applies a LASSO model to predict the annual cost
of claims incurred by the insurer (‘cost_claims_year’),
applying a logarithmic transformation to correct
for the asymmetry of the amounts.
"""
...
def run_lasso(X, y):
    ...
    lasso_cv = LassoCV(
        cv=CV_FOLDS, random_state=RANDOM_STATE, n_alphas=100, max_iter=5000, n_jobs=-1
    )
    lasso_cv.fit(X, y)
    ...
    plt.axvline(
        np.log10(lambda_min), linestyle="--", label=f"lambda.min={lambda_min:.4g}"
    )
    plt.xlabel("log10(lambda)")
    plt.ylabel("MSE (CV)")
    ...
    print(
        f"Nb de features non nulles : {(lasso_cv.coef_ != 0).sum()} / {len(lasso_cv.coef_)}"
    )
...
def main():
    ...
    r2 = (
        1 - ((y_target - y_pred) ** 2).sum() / ((y_target - y_target.mean()) ** 2).sum()
    )
