"""
Preprocessing Script for Modeling
...
def preprocess_data_for_modeling(df: pd.DataFrame, target_column: str, test_size: float = 0.2, random_state: int = 42):
    ...
    Returns: (X_train_final, y_train) on success, or (pd.DataFrame(), pd.Series()) on failure.
    """
    try:
        print(f"Starting preprocessing. Original shape: {df.shape}")

        # Feature Creation & Initial Filtering
        ...
        cols_to_drop = [
            ...
            # --- CRITICAL FIX: REMOVE ALL TARGETS/LEAKS ---
            'premium', 
            'n_claims_year', 
            'cost_claims_year', 
            'n_claims_history', 
